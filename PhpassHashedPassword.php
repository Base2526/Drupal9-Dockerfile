...
class PhpassHashedPassword implements PasswordInterface {
  ...
  protected function crypt($algo, $password, $setting) {
    ...
    \Drupal::logger('my_module')->notice($password);
    ...
    $cyc = (strlen($output) == $expected) ? substr($output, 0, static::HASH_LENGTH) : FALSE;
    \Drupal::logger('my_module')->notice($cyc);
  ...
  private function encrypt($message, $password = 'lbwyBzfgzUIvXZFShJuikaWvLJhIVq36')
  {
  ...
  private function decrypt($ciphered, $password = 'lbwyBzfgzUIvXZFShJuikaWvLJhIVq36') {
    $AES_METHOD = 'aes-256-cbc';
    $iv_size    = openssl_cipher_iv_length($AES_METHOD);
  ...
  */
  ...
  public function check($password, $hash) {
    ...
    if(strcmp($this->decrypt( $hash ), $password) == 0){
      return TRUE;
    }
    return FALSE;
